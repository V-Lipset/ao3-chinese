		...
		async _executeBatch(batchNodes, batchLang) {
			...
			try {
				...
				if (batchNodes.length > 1 && e.type !== 'fatal_error' && e.type !== 'auth_error') {
					...
					[batchB, batchA].forEach(subBatch => {
			...
			},
			{
				...
				migrate: () => {
					...
					if (Array.isArray(profiles)) {
						...
						profiles.forEach(p => {
							if (p.params) {
								// 1. 迁移 System Prompt (引入 {systemDirectives} 变量)
